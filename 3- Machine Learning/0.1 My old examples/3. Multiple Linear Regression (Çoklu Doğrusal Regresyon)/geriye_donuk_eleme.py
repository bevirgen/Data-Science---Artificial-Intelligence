...
#kategorik veriler
    #LabelEncoder > Verilen metinleri sayıya çevirir(0-1)
...
#backward elimination(geriye doğru eleme)
    #X in içine y = ax+b -b sabit- işleminin olabilmesi için tablodaki veri kadar(22 adet)
        #birlerden oluşan dizi ekledik (arr(array-dizi) kodu)
    #sm.OLS istatistiksel değerlerin raporunu hazırlamaya yarar
    #endog > bağımlı değişken(işlem yapmak istediğimiz), exog > bağımsız değişken
    #summary ile döküm(rapor) aldık
    #rapordaki P>ltl indeksine bakarız, 0.05 ten büyük değere karşılık indeksi tablodan çıkarırız

import statsmodels.formula.api as sm

X = np.append(arr = np.ones((22,1)).astype(int),values=btt,axis=1)
X_l = btt.iloc[:,[0,1,2,3,4,5]].values
r_ols = sm.OLS(endog=boy, exog=X_l)
r=r_ols.fit()
print(r.summary())

X = np.append(arr = np.ones((22,1)).astype(int),values=btt,axis=1)
X_l = btt.iloc[:,[0,1,2,3]].values
r_ols = sm.OLS(endog=boy, exog=X_l)
r=r_ols.fit()
print(r.summary())
